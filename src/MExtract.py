...
class procAreaInFile:
    ...
    def processArea(
        ...
        r = None
        try:
            r = self.processAreaFor(
                oldData[self.colInd[colmz]],
                oldData[self.colInd[colrt]],
                ppm,
                scanEvent,
                maxRTShift,
                smoothingWindow,
            ...
            logging.error(
                "   - Reintegration failed for feature pair (N) %s (%s %s) [%s]"
                % (
                    self.forFile,
                    oldData[self.colInd[colmz]],
                    oldData[self.colInd[colrt]],
        ...
        try:
            ...
            logging.error(
                "   Reintegration failed for feature pair (L) %s (%s %s) [%s]"
                % (
                    self.forFile,
                    oldData[self.colInd[colmz]],
                    oldData[self.colInd[colrt]],
    ...
    def processFile(
        ...
        for oDat in oldData:
            if self.queue is not None:
                self.queue.put(Bunch(pid=self.pID, mes="value", val=z))
            z += 1
        ...
        logging.info("   Reintegration finished for file %s (%.1f minutes)" % (self.forFile, (time.time() - startProc) / 60.0))
    ...
    def processFileParamed(self):
        ...
        if not USEGRADIENTDESCENDPEAKPICKING:
            ...
            self.CP = GradientPeaks(minInt=10000, minIntFlanks=10, minIncreaseRatio=0.05, expTime=[25, 250])  ## Swiss Orbitrap HF data
            self.CP = GradientPeaks(minInt=1000, minIntFlanks=10, minIncreaseRatio=0.15, expTime=[10, 150])
        ...
        self.processFile(
            ...
            self.negativeScanEvent,
            self.ppm,
            self.maxRTShift,
            self.smoothingWindow,
            self.smoothingWindowSize,
            self.smoothingWindowPolynom,
        )
...
def integrateResultsFile(
    ...
    if pw is not None:
        pw.getCallingFunction()("max")(len(colToProc))
    ...
    if len(colToProcCur) > 0:
        _integrateResultsFile(
            ...
            cpus,
            pwOffset=pwOffset,
            totalFilesToProc=len(colToProc),
...
def writeReintegrateConfigToDB(
    curs,
    maxRTShift,
    negativeScanEvent,
    positiveScanEvent,
    ...
    SQLInsert(
        curs,
        "config",
        key="FPREINT_reintegrateIntensityCutoff",
        value=str(reintegrateIntensityCutoff),
    )
...
def _integrateResultsFile(
    ...
    if pw is not None:
        pw.getCallingFunction()("text")("Integrating missed peaks")
    if pw is not None:
        pw.getCallingFunction()("value")(0 + pwOffset)
    if pw is not None:
        pw.getCallingFunction()("max")(totalFilesToProc)
    ...
    if pw is not None:
        pw.setCloseCallback(closeCallBack=CallBackMethod(_target=interruptReIntegrateFilesProcessing, pool=p, selfObj=selfObj).getRunMethod())
    ...
    for col in colToProc:
        ...
        u.setParams(
            file + ".tmp.tsv",
            [x.getName() for x in table.getColumns()],
            s,
            colmz,
            colrt,
            colxcount,
    ...
    while loop and not selfObj.terminateJobs:
        ...
        if completed == len(colToProc):
            ...
            if pw is not None:
                pw.getCallingFunction()("value")(completed + pwOffset)
            if pw is not None:
